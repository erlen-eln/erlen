...
// ハッシュ連鎖の対象列を固定順の配列にした正規形（鍵順の曖昧さを消すため。この順序を変えないこと）。
...
export async function hashEvent(ev) {
  const digest = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(canonicalize(ev)));
...
// ev = { action, targetType, targetId, pageId?, before?, after?, reason? }
