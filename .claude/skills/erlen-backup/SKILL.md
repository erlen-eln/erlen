---
name: erlen-backup
description: 電子実験ノート（Erlen）のバックアップと復元を行うスキル。D1（実験記録・試薬・在庫・機器・改訂履歴・操作ログ）を wrangler d1 export で、R2の添付ファイルを attachments テーブルの台帳をもとに1件ずつローカルへ退避する。復元手順（破壊的なので必ず人の承認を取る）も含む。「バックアップを取って」「ノートを退避して」「復元して」「erlen backup」などで起動する。
---
...
| 対象 | 中身 | 保存先 |
|---|---|---|
| **D1** | 実験記録・分子・添付の台帳・改訂履歴・操作ログ・試薬・在庫・機器・メンバー | `backups/<日付>/erlen.sql` |
...
- **`page_revisions`（改訂履歴）と `audit_events`（操作ログ）を除外しない。**
  監査証跡はバックアップの一部（記録をいつまで残すかの要求に関わる）
